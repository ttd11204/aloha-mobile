...
export function SideQuestLayout({ challenges, bonusChallenge }: SideQuestProps) {
  const [selectedChallenge, setSelectedChallenge] = useState<TreasureHuntItem | null>(null);
          ...
          <Animated.View className="rounded-lg bg-amber-50 border border-amber-300 overflow-hidden w-full max-w-[500px]">
            <View className="p-4">
              <View className="flex-row justify-between items-center mb-2">
                <Text className="text-lg font-bold text-amber-800">Side Quest of the Day</Text>
                <View className="bg-amber-500 px-3 py-1 rounded-2xl">
                  <Text className="text-white font-semibold text-xs">{bonusChallenge.points} pts</Text>
                </View>
              </View>
              <Text className="text-gray-600 text-base">{bonusChallenge.text}</Text>
            </View>
          </Animated.View>
        </View>
      )}
        ...
        {challenges.map((challenge) => (
          ...
          >
            <View className="p-4 min-h-[160px] justify-between">
              <Text className="text-base font-semibold mb-2 text-gray-800">{challenge.title}</Text>
              <Text className="text-sm italic text-gray-600 flex-1 mb-3">{challenge.description}</Text>
              <View className="flex-row justify-between items-center mt-2">
                <Text className="text-xs text-gray-400">Click for details</Text>
                <View className="bg-amber-100 px-2 py-1 rounded">
                  <Text className="text-amber-800 font-semibold text-xs">{challenge.points} pts</Text>
                </View>
              </View>
            </View>
          </TouchableOpacity>
            ...
            {selectedChallenge && (
              <>
                <View className="flex-row justify-between items-center mb-4">
                  <Text className="text-lg font-bold text-amber-800 flex-1">{selectedChallenge.title}</Text>
                  <View className="bg-amber-500 px-3 py-1 rounded-2xl">
                    <Text className="text-white font-semibold text-xs">{selectedChallenge.points} pts</Text>
                  </View>
                </View>
                  ...
                  {selectedChallenge.requirement
                    ...
                    .map((outcome, idx) => (
                      <View key={idx} className="flex-row items-start mb-1.5">
                        <Text className="text-emerald-600 mr-2 font-bold">✓</Text>
                        <Text className="text-sm text-gray-800 flex-1">{outcome}</Text>
                      </View>
                    ))}
                  {selectedChallenge.requirement &&
                    selectedChallenge.requirement.length > (windowWidth < 380 ? 2 : 3) && (
                    <Text className="text-violet-600 text-sm mt-1">
                      + {selectedChallenge.requirement.length - (windowWidth < 380 ? 2 : 3)} more
                    </Text>
                  )}
                </ScrollView>
                ...
                <View className="mb-4">
                  <Text className="text-base font-semibold text-gray-600 mb-2">Upload Image/Video:</Text>
                  
                  <TouchableOpacity 
                    onPress={pickImage}
                    className="flex-row items-center justify-center bg-amber-100 p-3 rounded-lg mb-2"
                  >
                    <MaterialIcons name="file-upload" size={24} color="#92400E" />
                    <Text className="ml-2 text-amber-800 font-medium">Select from gallery</Text>
                  </TouchableOpacity>
                  ...
                  {previewUrl && (
                    <View className="mt-3">
                      ...
                      {selectedFile?.type?.startsWith('image') || (!selectedFile?.type && selectedFile?.uri?.match(/\.(jpg|jpeg|png|gif)$/i)) ? (
                        <Image
                ...
                >
                  {isLoading ? (
                    ...
                    <Text className="text-amber-800 font-semibold text-base">Submit</Text>
                  )}
                </TouchableOpacity>
                ...
                >
                  <Text className="text-amber-800 font-semibold text-base">Close</Text>
                </TouchableOpacity>
              </>
            )}
          </Animated.View>
        ...
        <View className="flex-1 bg-black/50 justify-center items-center p-4">
          ...
          >
            ...
            <View className="items-center mb-6">
              <Text className="text-2xl font-bold text-gray-800 mb-2">Upload Successful!</Text>
            ...
            >
              <Text className="text-amber-800 font-semibold text-base">Continue to other quests</Text>
            </TouchableOpacity>
          </Animated.View>
        </View>
      </Modal>
